...
The core axioms are ⟨proof⟩ objects that prove tautologies.
They are usually parametrized by ⟨term⟩ objects.  We distinguish the following
categories.
...
positive polarity.  They are also identical to the clauses produced
by Tseitin encoding.  The following lists the axioms and the
corresponding clause proved by each axiom.  In this list `t, t0, …`
...
For efficient `xor` reasoning we allow arbitrary splitting and reordering
...
contain each term an even number of times.  Furthermore, if one of
...
quantified variables either to an arbitrary term or via the
...
PAIRWISE attribute and more than two arguments, `expand` applies
