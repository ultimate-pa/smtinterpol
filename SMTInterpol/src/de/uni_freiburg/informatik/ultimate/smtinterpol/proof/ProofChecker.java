...
public class ProofChecker extends NonRecursive {
	...
	static class ResolutionWalker implements Walker {
		...
		public void walk(final NonRecursive engine) {
			...
			for (int i = subProofs.length - 1; i >= 0; i--) {
	...
	static class InstLemmaWalker implements Walker {
		...
		public void walk(final NonRecursive engine) {
			...
			final Term[] provedClause = checker.stackPop();
			if (provedClause == null || provedClause.length != 1) {
				checker.reportError("Lemma :inst needs subproof for term equality.");
				return;
			}
			final Term proved = provedClause[0];
	 ...
	 */
	private void collectEqualities(final Term[] clause, final Set<SymmetricPair<Term>> equalities,
			final Set<SymmetricPair<Term>> disequalities) {
		for (final Term literal : clause) {
			...
			if (negated) {
				...
				equalities.add(new SymmetricPair<>(sides[0], sides[1]));
			} else {
				disequalities.add(new SymmetricPair<>(sides[0], sides[1]));
			}
		}
	}

	/**
	 * Check a CC lemma for correctness. If a problem is found, an error is
	 * reported.
	 *
	...
	private void checkDataTypeLemma(final String type, final Term[] clause, final Object[] ccAnnotation) {
		...
		case ":dt-project": {
			...
			final Term goalEquality = (Term) ccAnnotation[0];
			if (!isApplication("=", goalEquality)) {
				reportError("malformed main equality in dt-project");
			}
			if (!ccAnnotation[1].equals(":cons")) {
				reportError(":cons annotation missing in dt-project");
			}
			...
			if (!consFunc.isConstructor()) {
				reportError("constructor term expected in :cons annotation");
			}
			final Term[] goalTerms = ((ApplicationTerm) goalEquality).getParameters();
			if (goalTerms.length != 2) {
				reportError("malformed main equality in dt-project");
			...
			if (!(goalTerms[0] instanceof ApplicationTerm)) {
				reportError("select term expected in dt-project");
			}
			...
			if (goalTerms[1] != consArg) {
				reportError("malformed main equality in dt-project");
			}

			if (!allDisequalities.contains(new SymmetricPair<>(selectTerm, consArg))
					&& !checkTrivialDisequality(selectTerm, consArg)) {
				reportError("missing main equality in dt-project");
			}
			if (!allEqualities.contains(new SymmetricPair<>(selectArg, consTerm))
					&& !checkTrivialEquality(selectArg, consTerm)) {
				reportError("missing cons equality in dt-project");
			}
			break;
		}
		case ":dt-tester": {
			...
			final Term goalEquality = (Term) ccAnnotation[0];
			if (!isApplication("=", goalEquality)) {
				reportError("malformed main equality in dt-tester");
			}
			if (!ccAnnotation[1].equals(":cons")) {
				reportError(":cons annotation missing in dt-tester");
			}
			final ApplicationTerm consTerm = (ApplicationTerm) ccAnnotation[2];
			final FunctionSymbol consFunc = consTerm.getFunction();
			if (!consFunc.isConstructor()) {
			...
			if (!isApplication(SMTLIBConstants.IS, goalTerms[0])) {
				reportError("select term expected in dt-tester");
			}
			...
			if (!isApplication((isSameCons ? SMTLIBConstants.TRUE : SMTLIBConstants.FALSE), goalTerms[1])) {
				reportError("malformed main equality in dt-tester");
			}

			if (!allDisequalities.contains(new SymmetricPair<>(goalTerms[0], goalTerms[1]))
					&& !checkTrivialDisequality(goalTerms[0], goalTerms[1])) {
				reportError("missing main equality in dt-tester");
			}
			if (!allEqualities.contains(new SymmetricPair<>(testerArg, consTerm))
					&& !checkTrivialEquality(testerArg, consTerm)) {
				reportError("missing cons equality in dt-tester");
			}
			break;
		}
		case ":dt-constructor": {
			...
			final Term goalEquality = (Term) ccAnnotation[0];
			if (!isApplication("=", goalEquality)) {
				reportError("malformed main equality in dt-constructor");
			}
			final Term[] goalTerms = ((ApplicationTerm) goalEquality).getParameters();
			if (goalTerms.length != 2) {
				reportError("malformed main equality in dt-constructor");
			...
			if (!(goalTerms[1] instanceof ApplicationTerm)) {
				reportError("select term expected in dt-constructor");
			}
			final ApplicationTerm consTerm = (ApplicationTerm) goalTerms[1];
			if (!consTerm.getFunction().isConstructor()) {
				reportError("constructor term expected in dt-constructor");
			}
			...
			final Term testerTerm = mSkript.term(SMTLIBConstants.IS, new String[] { cons.getName() }, null, dataTerm);
			...
			final Term[] consArgs = consTerm.getParameters();
			final String[] selects = cons.getSelectors();
			for (int i = 0; i < consArgs.length; i++) {
				final Term selectTerm = mSkript.term(selects[i], dataTerm);
				if (!allEqualities.contains(new SymmetricPair<>(consArgs[i], selectTerm))
						&& !checkTrivialEquality(consArgs[i], selectTerm)) {
					reportError("missing is equality in dt-constructor");
				}
			}
			if (!allEqualities.contains(new SymmetricPair<>(testerTerm, trueTerm))) {
				reportError("missing is condition in dt-constructor");
			}
			break;
		}
		case ":dt-cases": {
			...
			for (int i = 0; i < ccAnnotation.length; i++) {
				...
				final Term arg = tester.getParameters()[0];
				if (!allEqualities.contains(new SymmetricPair<>(arg, firstData))
						&& !checkTrivialEquality(arg, firstData)) {
					reportError("missing arg equality in clause in dt-cases");
				}
			}
			break;
		}
		case ":dt-unique": {
			...
			for (int i = 0; i < ccAnnotation.length; i++) {
				final ApplicationTerm tester = (ApplicationTerm) ccAnnotation[i];
				assert isApplication(SMTLIBConstants.IS, tester);
				if (!allEqualities.contains(new SymmetricPair<>(tester, trueTerm))) {
					reportError("missing test equality in clause in dt-unique");
				}

				final Term arg = tester.getParameters()[0];
				if (!allEqualities.contains(new SymmetricPair<>(arg, firstData))
						&& !checkTrivialEquality(arg, firstData)) {
					reportError("missing arg equality in clause in dt-unique");
				}
			}
			break;
		}
	...
	Term[] walkTautology(final ApplicationTerm tautologyApp) {
		/*
		 * Tautologies are created to define the meaning of proxy literals like (or a b
		 * c), or of proxy terms like (ite cond t1 t2) or (div x 5). They are of the
		 * form
		 *
		 * (@tautology (! (or ...) :type))
		 *
		...
		case ":trueNotFalse":
			...
			break;
		case ":false-":
			result = clause.length == 1 && clause[0] == mSkript.term("not", mSkript.term("false"));
			break;
		case ":true+":
			result = clause.length == 1 && clause[0] == mSkript.term("true");
			break;
		case ":or+":
		...
		case ":=-1":
		case ":=-2":
			result = checkTautIff(tautologyName, clause);
			break;
		case ":xor+1":
		case ":xor+2":
		case ":xor-1":
		...
		case ":forall-":
			...
			break;
		case ":exists-":
			result = checkTautExistsElim(clause, (Term[]) annot.getValue());
			break;
		case ":exists+":
			result = checkTautExistsIntro(clause, (Term[]) annot.getValue());
			break;
		default:
			result = false;
			break;
	...
	private boolean checkTautOrNeg(final Term[] clause) {
		// Check for the form: (or (not (or p1 ... pn)) p1 ... pn)
		Term lit = clause[0];
		if (isAuxEquality(lit)) {
			lit = expandAuxEquality((ApplicationTerm) lit, false);
		}
		lit = negate(lit);
	...
	private boolean checkTautOrPos(final Term[] clause) {
		// Check for the form: (or (or p1 ... pn) (not pi))
		if (clause.length != 2) {
			return false;
		}
		Term lit = clause[0];
	...
	private boolean checkTautAndNeg(final Term[] clause) {
		// Check for the form: (or (not (and p1 ... pn)) pi)
		if (clause.length != 2) {
			return false;
		}
		Term lit = clause[0];
	...
	private boolean checkTautAndPos(final Term[] clause) {
		// Check for the form: (or (and p1 ... pn) (not p1) ... (not pn))
		Term lit = clause[0];
		if (isAuxEquality(lit)) {
			lit = expandAuxEquality((ApplicationTerm) lit, false);
		}
	...
	private boolean checkTautImpNeg(final Term[] clause) {
		// Check for the form: (or (not (=> p1 ... pn)) (not p1) ... (not pn-1) pn)
		Term lit = clause[0];
		if (isAuxEquality(lit)) {
			lit = expandAuxEquality((ApplicationTerm) lit, false);
		}
	...
	private boolean checkTautImpPos(final Term[] clause) {
		// Check for the form: (or (=> p1 ... pn) pi), resp ... (not pn))
		if (clause.length != 2) {
			return false;
		}
		Term lit = clause[0];
	...
	private boolean checkTautIff(final String tautKind, final Term[] clause) {
		if (clause.length != 3) {
			return false;
		}
		Term lit = clause[0];
		...
		case ":=+1":
			...
			return !negated && clause[1] == eqParams[0] && clause[2] == eqParams[1];
		case ":=+2":
			// (or (= t1 t2) (not t1) (not t2))
			return !negated && clause[1] == mSkript.term("not", eqParams[0])
					&& clause[2] == mSkript.term("not", eqParams[1]);
		case ":=-1":
			// (or (not (= t1 t2) t1 (not t2))
			return negated && clause[1] == eqParams[0] && clause[2] == mSkript.term("not", eqParams[1]);
		case ":=-2":
			// (or (not (= t1 t2) (not t1) t2)
			return negated && clause[1] == mSkript.term("not", eqParams[0]) && clause[2] == eqParams[1];
		}
		return false;
	}
	...
	private boolean checkTautExcludedMiddle(final String name, final Term[] clause) {
		...
		// Check for the form: (or (= p true) (not p)) :excludedMiddle1
		// or (or (= p false) p) :excludedMiddle2
		Term equality = clause[0];
		if (isAuxEquality(equality)) {
			equality = expandAuxEquality((ApplicationTerm) equality, true);
		}
	 ...
	 * @return true iff the clause is well-formed.
	 */
	boolean checkTautForallElim(final Term[] clause, final Term[] subst) {
		mNumInstancesUsed++;
	 ...
	 * @return true iff the clause is well-formed.
	 */
	boolean checkTautExistsIntro(final Term[] clause, final Term[] subst) {
		...
		// subst must contain as many variables as the lhs has universally quantified
		// variables
	 ...
	 * @return true iff the clause is well-formed.
	 */
	boolean checkTautExistsElim(final Term[] clause, final Term[] skolemFuns) {
		...
		if (!(existsAtom instanceof QuantifiedFormula)) {
			return false;
		}
	...
	Term[] walkTransitivity(final ApplicationTerm transitivityApp, final Term[][] implications) {
		...
		for (int i = 0; i < implications.length; i++) {
			...
			if (isApplication("=>", impl)) {
				containsImplication = true;
			}
			final Term[] impParams = ((ApplicationTerm) impl).getParameters();
		...
		return new Term[] { transitivityApp.getTheory().term(containsImplication ? "=>" : "=", firstTerm, lastTerm) };
	}
	...
	Term[] walkCongruence(final ApplicationTerm congruenceApp, final Term[][] subProofs) {
		...
		final Term newEquality = theory.term("=", ((ApplicationTerm) subProofs[0][0]).getParameters()[0],
	...
	boolean checkRewriteAuxIntro(final Term lhs, final Term rhs) {
		return isAuxApplication(rhs) && expandAux((ApplicationTerm) rhs) == lhs;
	}
	...
	boolean checkRewriteIntern(final Term lhs, Term rhs) {
		...
		if (isAuxEquality(rhs)) {
			return lhs == expandAuxEquality((ApplicationTerm) rhs, false);
		}
	...
	/**
	 * Convert a term to a unit clause. This removes double negations.
	 *
	 * @param unit
	 * @return the unit clause.
	 */
	private Term[] createUnitClause(Term unit) {
		while (isDoubleNeg(unit)) {
			unit = removeDoubleNeg(unit);
		}
		return new Term[] { unit };
	}

	/**
	 * Convert an or-term to a clause. This also removes double negations.
	 *
	 * @param clauseAsTerm
	 * @return the clause.
	 */
	private Term[] createClause(final Term clauseAsTerm) {
		if (isApplication("false", clauseAsTerm)) {
			return new Term[0];
		}
		Term[] args;
		if (isApplication("or", clauseAsTerm)) {
			args = ((ApplicationTerm) clauseAsTerm).getParameters();
		} else {
			args = new Term[] { clauseAsTerm };
		}
		final LinkedHashSet<Term> clause = new LinkedHashSet<>();
		for (Term arg : args) {
			while (isDoubleNeg(arg)) {
				arg = removeDoubleNeg(arg);
			}
			clause.add(arg);
		}
		return clause.toArray(new Term[clause.size()]);
	}
	...
	Term[] walkResolution(final ApplicationTerm resApp, final Term[][] subProofs) {
		...
		allDisjuncts.addAll(Arrays.asList(subProofs[0]));
		...
		return allDisjuncts.toArray(new Term[allDisjuncts.size()]);
	}
	...
	Term[] walkModusPonens(final ApplicationTerm mpApp, final Term[] origFormula, final Term[] rewrite) {
		...
		boolean okay = false;
		Term[] result = null;
		if (rewrite != null && rewrite.length == 1
				&& (isApplication("=", rewrite[0]) || isApplication("=>", rewrite[0]))) {
			final Term[] mpSides = ((ApplicationTerm) rewrite[0]).getParameters();
			if (mpSides.length == 2) {
				result = createUnitClause(mpSides[1]);
				okay = origFormula != null && origFormula.length == 1 && (origFormula[0] == mpSides[0]);
			}
		}
	...
	Term[] walkClause(final ApplicationTerm clauseApp, final Term[] provedLits) {
		/* The first parameter of clause is a subproof for provedClause. */
		/*
	...
	void stackPush(final Term[] pushClause, final ApplicationTerm keyTerm) {
	...
	/**
	 * Check if the given atom term is an aux equality. This is an equality of the
	 * form (= (@AUX...) true) or (= (@AUX...) false).
	 *
	 * @param atom the term to check
	 * @return true if this is an AUX equality atom.
	 */
	private boolean isAuxEquality(final Term atom) {
		if (isApplication("=", atom)) {
			final ApplicationTerm auxApp = (ApplicationTerm) atom;
			final Term[] args = auxApp.getParameters();
			return isAuxApplication(args[0]) && (isApplication("true", args[1]) || isApplication("false", args[1]));
		}
		return false;
	}

	Term expandAuxEquality(final ApplicationTerm auxEquality, final boolean excludedMiddle) {
		assert isApplication("=", auxEquality);
		final Term rhs = auxEquality.getParameters()[1];
		final boolean isNegated = isApplication("false", rhs);
		assert isNegated || isApplication("true", rhs);
		final Term auxTerm = auxEquality.getParameters()[0];
		assert isAuxApplication(auxTerm);
		// the definition of the quantAuxLit can be found in the annotation
		Term definingTerm = expandAux((ApplicationTerm) auxTerm);
		if (excludedMiddle) {
	...
	/**
	 * Checks if a term is a double negated term.
	 * @param term
	 *            the term to check.
	 * @return true if term is an application of not on an application of not.
	 */
	private boolean isDoubleNeg(final Term term) {
		return isApplication("not", term)
				&& isApplication("not", ((ApplicationTerm) term).getParameters()[0]);
	}

	/**
	 * Removes double negation from a term.  This assumes it is a double negated term.
	 * @param term the double negated term.
	 * @return the term without double negation.
	 */
	private Term removeDoubleNeg(final Term term) {
		assert isDoubleNeg(term);
		return ((ApplicationTerm) ((ApplicationTerm) term).getParameters()[0]).getParameters()[0];
	}
