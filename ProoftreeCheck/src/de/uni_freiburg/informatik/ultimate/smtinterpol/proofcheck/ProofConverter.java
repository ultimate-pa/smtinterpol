...
class ProofConverter extends NonRecursive {
	...
	private void fillMap() {
		mProofNode2Method.put(ProofConstants.FN_RES, new ResNode());
		mProofNode2Method.put(ProofConstants.FN_LEMMA, new LemmaNode());
		mProofNode2Method.put(ProofConstants.FN_ASSERTED, new AssertedNode());
		mProofNode2Method.put(ProofConstants.FN_TAUTOLOGY, new TautologyNode());
		if (!mPartialProof) {
			mProofNode2Method.put(ProofConstants.FN_EQ, new EqNode());
			mProofNode2Method.put(ProofConstants.FN_SPLIT, new SplitNode());
			mProofNode2Method.put(ProofConstants.FN_CLAUSE, new ClauseNode());
		}
	}
	...
	public void convert(Term proof,
		...
		proof = new FormulaLet(
					new FormulaLet.LetFilter() {
						@Override
						public boolean isLettable(final Term term) {
							return ((term instanceof ApplicationTerm)
									&& ((ApplicationTerm)term).getFunction().
										getReturnSort().getName() == ProofConstants.SORT_PROOF);
						}
					}).let(proof);
	...
	private class ScopingLemmaWalker implements NonRecursive.Walker {
		 ...
		 */
		public ScopingLemmaWalker(final TermVariable lemmaVar) {
			mLemmaVar = lemmaVar;
		}
	...
	private class ProofWalker extends NonRecursive.TermWalker {
		 ...
		 */
		ProofWalker(final Term term, final TermVariable lemma) {
			super(term);
			mLemma = lemma;
		}
		 ...
		 */
		@Override
		public void walk(final NonRecursive converter, final ConstantTerm term) {
		 ...
		 */
		@Override
		public void walk(final NonRecursive converter, final AnnotatedTerm term) {
			throw new IllegalArgumentException(
		 ...
		 */
		@Override
		public void walk(final NonRecursive converter, final QuantifiedFormula term) {
		 ...
		 */
		@Override
		public void walk(final NonRecursive converter, final ApplicationTerm term) {
		...
		public void walk(final NonRecursive converter, final LetTerm term) {
			...
			class LetWalker implements NonRecursive.Walker {
				 ...
				 *        assigned to
				 */
				public LetWalker(final TermVariable variable) {
					mVariable = variable;
				}
			...
			for (int i = variables.length - 1; i >= 0; --i) {
				...
				if (values[i] instanceof ApplicationTerm) {
					...
					if (name == ProofConstants.FN_REWRITE) {
						converter.enqueueWalker(
								new EqualityWalker(aValue, true, variable));
					} else if (name == ProofConstants.FN_INTERN) {
						converter.enqueueWalker(
		 ...
		 */
		@Override
		public void walk(final NonRecursive converter, final TermVariable variable) {
	 ...
	 */
	private String convertRewrite(final Term[] parameters) {
	 ...
	 */
	private String convertIntern(final Term[] parameters) {
		assert ((parameters.length == 1)
	...
	private class ResNode implements IProofNode {
		 ...
		 */
		@Override
		public void convert(final NonRecursive converter, final Term[] parameters,
				final TermVariable lemmaVar) {
			/**
			...
			class ResolutionWalker implements NonRecursive.Walker {
				 ...
				 */
				public ResolutionWalker(final Term pivot) {
					mPivot = pivot;
				}
	...
	private class EqNode implements IProofNode {
		...
		public void convert(final NonRecursive converter, final Term[] parameters,
			...
			class SubstitutionWalker implements NonRecursive.Walker {
				 ...
				 */
				public SubstitutionWalker(final Term equalityNode) {
	...
	private class SplitNode implements IProofNode {
		...
		public void convert(final NonRecursive converter, final Term[] parameters,
			...
			class SplitWalker implements NonRecursive.Walker {
				 ...
				 */
				public SplitWalker(final ApplicationTerm result, final String annotation,
						final TermVariable splitLemmaVar) {
					mResult = result;
