...
public class Utils {
	...
	public Term convertNot(Term input) {
		ApplicationTerm notTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
	...
	public Term convertOr(Term input) {
		ApplicationTerm orTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
	...
	public Term convertLeq0(Term input) {
		ApplicationTerm leq0Term = (ApplicationTerm) mTracker.getProvedTerm(input);
	...
	public Term convertIte(Term input) {
		ApplicationTerm iteTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
		...
		if (trueBranch == theory.mFalse && falseBranch == theory.mTrue) {
			Term result = mTracker.transitivity(input, 
	...
	public Term convertBinaryEq(Term input) {
		...
		if (args[0].getSort().isArraySort()) {
			// Check store-rewrite
			if (isStore(args[0])) {
				ApplicationTerm store = (ApplicationTerm) args[0];
				if (args[1] == store.getParameters()[0]) {
					return convertStoreRewrite(input, store, false);
				}
			}
			if (isStore(args[1])) {
				ApplicationTerm store = (ApplicationTerm) args[1];
	...
	private Term convertStoreRewrite(Term input, ApplicationTerm store, boolean arrayFirst) {
		Term eqTerm = mTracker.getProvedTerm(input);
		...
		mTracker.buildRewrite(eqTerm, result, ProofConstants.RW_STORE_REWRITE);
		return result;
	}
	...
	public Term convertDistinct(Term input) {
		ApplicationTerm distinctTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
		...
		if (args[0].getSort() == theory.getBooleanSort()) {
			...
			Term rhs = theory.term("=", t0, t1);
			Term rewrite = mTracker.transitivity(input, 
	...
	public Term convertFuncNot(Term input) {
		ApplicationTerm appTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
	...
	public Term convertNotOrNot(Term input) {
		...
		Term orRewrite = convertOrNot(mTracker.reflexivity(orTerm));
		return convertNot(mTracker.congruence(input, new Term[] { orRewrite }));
	}

	public Term convertAnd(Term input) {
		ApplicationTerm andTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
		assert andTerm.getFunction().getName() == "and";
		Theory theory = input.getTheory();
		Term[] args = andTerm.getParameters();
		Term[] negArgs = new Term[args.length];
		for (int i = 0; i < args.length; i++) {
			negArgs[i] = theory.term("not", args[i]);
		}
		Term notOrTerm = theory.term("not", theory.term("or", negArgs));
		Term andRewrite = mTracker.buildRewrite(andTerm, notOrTerm, ProofConstants.RW_AND_TO_OR);
	...
	public Term convertXor(Term input) {
		...
		Term rhs = theory.term("distinct", xorTerm.getParameters());
		Term xorRewrite = mTracker.buildRewrite(xorTerm, rhs, ProofConstants.RW_XOR_TO_DISTINCT);
		return convertDistinct(mTracker.transitivity(input, xorRewrite));
	}

	public Term convertImplies(Term input) {
		ApplicationTerm impliesTerm = (ApplicationTerm) mTracker.getProvedTerm(input);
		Theory theory = input.getTheory();
		assert impliesTerm.getFunction().getName() == "=>";
		Term[] args = impliesTerm.getParameters();
		...
		Term rhs = theory.term("or", newArgs);
		Term impliesRewrite = mTracker.buildRewrite(impliesTerm, rhs, ProofConstants.RW_IMP_TO_OR);
