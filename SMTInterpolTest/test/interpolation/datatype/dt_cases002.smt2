...
(assert (! (and (= (false) ((_ is nil) u)) (not ((_ is cons) u)) :named A ))
(assert (! (not ((_ is cons2) u)) :named C ))
(assert (! (not ((_ is cons3) u)) :named B ))

(check-sat)(set-option :produce-interpolants true)
(set-option :interpolant-check-mode true)
...
(assert (! (and (= ((_ is nil) u) false) (not ((_ is cons) u)) ) :named A ))
