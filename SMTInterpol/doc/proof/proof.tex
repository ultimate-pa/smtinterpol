...
\verb+(@rewrite bool @Proof)+ that takes the equality $t_1=t_2$ and
returns a proof of the equality.  The equality is annotated by the
rewrite rule, which explains why $t_1$ and $t_2$ should be equal. In the proof tree they appear as
...
\begin{align}
  ...
  \tag{distinctSame}\\
  (distinct\ t_1\ (not\ t_2)) &= true
  \quad \text{and similar if the arguments are swapped}
  \tag{distinctNeg}\\
  (distinct\ true\ t_2) &= (not\ t_2)
  \quad \text{and similar if the arguments are swapped}
  \tag{distinctTrue}\\
  (distinct\ false\ t_2) &= t_2
  \quad \text{and similar if the arguments are swapped}
  \tag{distinctFalse}\\
  (distinct\ t_1\ t_2)) &= (=\ (not\ t_1)\ t_2)
  \quad \text{or}\quad (=\ t_1\ (not\ t_2))
  \tag{distinctBoolEq}\\
  (=\ t_1 \cdots t_n) &= (not\ (or\ \dots (=\ t_i\ t_j)\dots))
  \quad\text{where $n \geq 2$, $1 \leq i < j \leq n$}
  \tag{distinctBinary}
\end{align}
The last rule is used to convert distinct applications into negated binary
equalities.  For $n=2$ the $or$ is omitted.
...
The canonical form is achieved by moving multiplicative constants
before the term and moving the divisor into the constant for division.
For addition, the constants for the same term are added together.
Usual arithmetic on rational numbers is performed to the constants.

As example, consider $t_1 = (+\ (*\ 3\ x)\ (*\ 2\ y))$ and
$t_2 = (+\ (*\ (- 3)\ x)\ y\ 1)$.  These terms are in canonical form. The
canonical form of $t_1+t_2$ is $(+\ (*\ 3\ y)\ 1)$ as the factors of $x$
cancel out.  The canonical form of $-t_2$ is $(+\ (*\ 3\ x)\ (-\ y)\ 1)$ or
$(+\ (-\ y)\ (*\ 3\ x)\ 1)$ as the order of the terms $x$ and $y$ is not
fixed.
...
\begin{align}
  ...
  \quad
  \text{where }
  \text{\begin{tabular}{l}$t_1,\dots,t_n$ are in canonical form,\\
      $t$ is the canonical form of the right hand side,\\
      for multiplication all but one $t_i$ must be a constant,\\
      for division all but the first $t_i$ must be a constant.
      \end{tabular}}
  \tag{canonicalSum}
\end{align}

For $to\_real$, the canonical form pushes this function into the summands,
converting all integer constants to real constants.

The comparison operators are normalized to less equal with second argument 0.
Here $(t_1 - t_2)$ stands for the canonical form of $(+ t_1 (- t_2))$.
\begin{align}
  (<=\ t_1\ t_2) &= (<=\ (t_1-t_2)\ 0) \tag{leqToLeq0}\\
  (<\ t_1\ t_2)  &= (not\ (<=\ (t_2-t_1)\ 0)) \tag{ltToLeq0}\\
  (>=\ t_1\ t_2) &= (<=\ (t_2-t_1)\ 0) \tag{geqToLeq0}\\
  (>\ t_1\ t_2)  &= (not\ (<=\ (t_1-t_2)\ 0)) \tag{gtToLeq0}
\end{align}
...
\begin{align}
  (div\ t\ 1) &= c \tag{div1}\\
  (div\ t\ (-\ 1)) &= -t\quad \text{where $-t$ is in canonical form} \tag{div-1} \\
  (div\ c_1\ c_2) &= c\quad \text{where $c = sgn(c_2) \lfloor c_1/|c_2| \rfloor$} \tag{divConst}
\end{align}
...
\begin{align}
  (mod\ t\ 1) &= 0 \tag{div1}\\
  (mod\ t\ (- 1)) &= 0 \tag{div-1} \\
  (mod\ c_1\ c_2) &= c \quad\text{where $c = c1 - |c_2|\lfloor c_1/|c_2| \rfloor$} \tag{moduloConst} \\
  (mod\ t_1\ c_2) &= t \quad\text{where $c_2\neq 0$ and $t$ is canonical form of $t_1 - c_2 (div\ t_1\ c_2)$} \tag{modulo}
\end{align}
For $c_2 = 0$ no rewrite is done and $mod$ behaves like an uninterpreted function.
...
\begin{equation}
  (to\_int\ c) = \lfloor c \rfloor \quad\text{where $c$ is constant} \tag{toInt}
\end{equation}
